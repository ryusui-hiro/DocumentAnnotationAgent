...
- Edit a candidate's label and note before confirming it. Corrections default to that candidate only; explicitly choose to apply the decision as a rule for remaining pages. Multiple decisions on a blocked page are kept in order, and only decisions marked as remaining-page rules are reused as guidance. Run history records each human action, its source candidate, scope, and versioned rule's first applicable page, alongside checked, image-only, opened, failed, and unprocessed page coverage.
