...
- 確認候補のラベルとメモを修正して確定。初期設定はその候補だけに適用し、残りのページにも判断ルールとして使う場合は明示的に選択。中断したページの複数判断は順番を保ち、後続ページへ渡すのは「残りのページにも適用」とした判断のみ。作業履歴に確認済み・画像のみ・未確認・失敗・未処理のページを記録し、ページ移動と、確認漏れや変換警告のあるページの1ページ再確認を提供します。
